...
import type { WP_REST_API_Post, WP_REST_API_Attachment, WP_REST_API_Tag } from 'wp-types'
...
export const createPostsFetcher =
  ...
  async ({ limit = 6, tag = '' } = {}): Promise<Post[]> => {
    const fetchApi = async <T>(endpoint: string, params?: URLParamaters): Promise<T> => {
    ...
    const posts = await fetchApi<WP_REST_API_Post[]>('/posts', {
      ...
      _fields: 'id,title,link,date,tags,_links.wp:featuredmedia,_embedded.wp:featuredmedia',
      per_page: limit,
    })
    ...
    return posts
      ...
      .map<Post>(({ id, title, link, date, _embedded }) => {
        ...
        return {
          ...
          image: (media.media_details.sizes as MediaSizes)[mediaTargetSize].source_url,
        }
      })
  }
